...
router.post("/board/login", function (req, res, next) {
  var mb_email = req.body.mb_email;
  var mb_password = req.body.mb_password;

  var query = "SELECT salt, mb_password FROM member WHERE mb_email=?";
  db.query(query, [mb_email], function (err, rows) {
    if (err) throw err;
    else {
      if (rows.length === 0) {
        ...
        var dbPassword = rows[0].mb_password;
        const hashPassword = crypto
          .createHash("sha512")
          .update(mb_password + salt)
          .digest("hex");
